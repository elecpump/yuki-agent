...
class WireRuntimeBusAdapter:
    """Give standalone agents the native RuntimeBus surface over an existing BusNode."""
    ...
    def error_count(self) -> int:
        return self._bus.error_count

    @property
    def dropped_count(self) -> int:
        return getattr(self._bus, "dropped_count", 0)

    def publish(self, topic: str, payload: Any, *, trace_id: str | None = None) -> None:
        self._bus.publish(
            topic,
            self._codec.encode_event(topic, payload),
            trace_id=trace_id,
        )
    ...
    def subscribe(self, topic_prefix: str, handler: EventHandler) -> None:
        self._bus.subscribe(
            topic_prefix,
            lambda topic, payload: handler(
                topic,
                self._codec.decode_event(topic, payload),
            ),
        )
    ...
    def respond(
        ...
        *,
        ...
        def wrapped(payload: dict) -> dict:
            local_payload = self._codec.decode_service_request(service, payload)
            result = handler(local_payload)
            return self._codec.encode_service_response(service, result)

        self._bus.respond(service, wrapped, lane=lane)

    def request(self, service: str, payload: dict, timeout_ms: int = 2000) -> dict:
        result = self._bus.request(
            service,
            self._codec.encode_service_request(service, payload),
            timeout_ms=timeout_ms,
        )
        return self._codec.decode_service_response(service, result)

    def pause_subscriptions(self) -> None:
        self._bus.pause_subscriptions()

    def resume_subscriptions(self) -> None:
        self._bus.resume_subscriptions()

    def bus_health(self) -> dict:
        return self._bus.bus_health()
