...
class ThreadTurnStore:
    ...
    def projection_items(self) -> tuple[list[dict], list[dict]]:
        with self._lock:
            ...
            fallback_rows = self._conn.execute(
                ...
                WHERE t.thread_id = 1
                  AND s.state = 'closed'
                  AND s.summary_state IN ('pending', 'placeholder')
                ORDER BY t.id DESC
                """
            ).fetchall()
        return (
    ...
    def _recover_abandoned_maintenance(self, now: float) -> None:
        self._conn.execute(
            "UPDATE segments SET summary_state = 'pending' WHERE summary_state = 'running'"
        )
        self._conn.execute(
            """
    ...
    def _active_episode(self, at: float) -> int:
        ...
        if row is not None:
            ...
            self._conn.execute(
                """
                UPDATE episodes
                SET state = 'closed', ended_at = last_activity_at
                WHERE id = ?
                """,
                (int(row["id"]),),
            )
