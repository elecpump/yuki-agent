...
def test_crisis_loop_uses_crisis_mode_and_interrupt_falls_back(tmp_path):
    ...
    router = FakeRouter(RouterDecision(GateRoute.CLOUD, 1.0, crisis=True))
    hub = DecisionHub(bus, memory=memory, loop=loop, local_router=router, local_enabled=True)
...
def test_cloud_reply_uses_router_judged_emotion(tmp_path):
    bus = FakeBus()
    memory = MemoryManager(MemoryStore(tmp_path / "m.db"))
    decision = RouterDecision(GateRoute.CLOUD, 0.9, emotion="anger")
    hub = DecisionHub(
        bus,
        memory=memory,
...
def test_crisis_uses_router_decision_and_static_fallback(tmp_path):
    bus = FakeBus()
    memory = MemoryManager(MemoryStore(tmp_path / "m.db"))
    router = FakeRouter(RouterDecision(GateRoute.CLOUD, 1.0, crisis=True))
...
def test_crisis_words_follow_model_route_when_not_flagged(tmp_path):
    bus = FakeBus()
    memory = MemoryManager(MemoryStore(tmp_path / "m.db"))
    decision = RouterDecision(GateRoute.LOCAL, 0.9)
    composer = FakeComposer(reply="local reply")
    hub = DecisionHub(
        bus,
        memory=memory,
        local_router=FakeRouter(decision),
    ...
    hub.on_user_utterance(Topics.USER_UTTERANCE, {"text": "我不想活了"})
    assert _reply_text(bus) == "local reply"
    assert composer.calls[0][0] == "我不想活了"
    memory.close()
